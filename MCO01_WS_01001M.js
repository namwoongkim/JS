...
//공통
var MCO01_WS_01001M = /** @class */ (function () {
    function MCO01_WS_01001M(app, http, logger, globalContext, navCtrl, dialogService, commonService, navParams) {
        this.http = http;
        this.logger = logger;
        ...
        this.rcvNo = ''; //접수번호
        this.rcvNoResult = ''; //접수번호-변경확인
        this.dsrnk = ''; //재해서열
        this.dsrnkList = []; //재해서열 리스트
        this.plrnk = ''; //증권서열
        this.hmgdCfcd = ''; //인물구분코드
        this.clcfCd = ''; //보상구분코드
        this.clcfList = []; //보상구분리스트
        this.selectedDsrnk = ''; //선택된 재해서열
        this.selectedClcf = ''; //선택된 보상구분
        this.formCd = ''; //서류코드
        this.formText = '추가서류';
        this.sendTab = 'custom';
    ...
    MCO01_WS_01001M.prototype.resetData = function (param) {
        this.rcvNo = param;
        this.dsrnk = '';
        this.dsrnkList = [];
        this.plrnk = '';
     ...
     * 서식/출력발송 진입
     */
    MCO01_WS_01001M.prototype.ngOnInit = function () {
    ...
    MCO01_WS_01001M.prototype.onSrchOptionArea = function (cd) {
    ...
    //검색방법 선택 Layer호출
    MCO01_WS_01001M.prototype.srchMthdChoc = function () {
        var _this = this;
        var params = {};
        this.dialogService.showModalDialog(MCO01_CO_01004L, params).then(function (result) {
            _this.logger.debug("Layer Result::::::::::::::::::" + JSON.stringify(result));
            if (result.rcvNo) {
                _this.rcvNo = result.rcvNo;
                _this.onRcvNo();
            }
        });
    };
     ...
     * 조회
     */
    MCO01_WS_01001M.prototype.onRcvNo = function () {
        var _this = this;
        var str = this.rcvNo;
        ...
        this.rcvNo = ret;
        var dto = new RcvNoInqCDTO();
        dto.rcvNo = this.rcvNo;
        this.resetData(dto.rcvNo);
        this.http.sendXml(dto, 'APP_CO', 'ItgrcNoInqPC', 'getListRcvNo', 'MCO01_WS_01001T', RcvNoInqPDTO)
            .subscribe(function (result) {
            _this.logger.debug(result);
            if ('MIFI0005' == result.proframeHeader.pfmResponseCode) {
                //값이없으면
                _this.resetData('');
                _this.dialogService.alert('접수번호가 잘못 입력되었습니다.');
                return;
            }
            else {
                ...
                if (0 == rcvNoInqDTO[0].rcvNo.length) {
                    ...
                    _this.rcvNoResult = _this.rcvNo;
                    _this.globalContext.rcvNo = _this.rcvNoResult;
                    _this.searchDsrnkList();
                }
            }
        });
    };
     ...
     * 재해서열 조회(접수번호에 속해있는)
     */
    MCO01_WS_01001M.prototype.searchDsrnkList = function () {
        var _this = this;
        if (this.rcvNo.trim() == '') {
            this.resetData('');
            ...
            var dto = new CoInqCDTO();
            //실행을 위한 dto  설정           
            dto.rcvNo = this.rcvNo.replace('-', '');
            //dto.hmgdCfcd = '02';
            this.http.sendXml(dto, 'APP_CO', 'CoKeyInqPC', 'getListDsrnk', 'MCO01_WS_01001T', CoInqPCDTO, { ignoreError: true })
                .subscribe(function (result) {
                _this.logger.debug(result);
                if ('MIFI0001' == result.proframeHeader.pfmResponseCode) {
                    _this.dsrnkList = Array.isArray(result.body.coInqCDTO) ? result.body.coInqCDTO : [result.body.coInqCDTO];
                    //this.hmgdCfcd = this.dsrnkList[0].hmgdCfcd;
                    //this.dsrnk = this.dsrnkList[0].dsrnk;
                    //this.clcfCd =  this.dsrnkList[0].clcfCd;
                    var selectedIndex = 0;
                    if (_this.dsrnkList.length > 1) {
                        for (var i = 0; i < _this.dsrnkList.length; i++) {
                            if (_this.empType == '01' && _this.dsrnkList[i].hmgdCfcd == '01') { //인서열
                                selectedIndex = i;
                                break;
                            }
                            else if (_this.empType == '02' && _this.dsrnkList[i].hmgdCfcd == '02') { //물서열
                                selectedIndex = i;
                                break;
                            }
                        }
                    }
                    _this.hmgdCfcd = _this.paramFlag && _this.navParams.data.hmgdCfcd ? _this.navParams.data.hmgdCfcd : _this.dsrnkList[selectedIndex].hmgdCfcd;
                    _this.dsrnk = _this.paramFlag && _this.navParams.data.dsrnk ? _this.navParams.data.dsrnk : _this.dsrnkList[selectedIndex].dsrnk;
                    _this.clcfCd = _this.paramFlag && _this.navParams.data.clcfCd ? _this.navParams.data.clcfCd : _this.dsrnkList[selectedIndex].clcfCd;
                    _this.selectedDsrnk = _this.hmgdCfcd + '' + _this.dsrnk;
                    _this.searchPlrnkList();
                }
            });
        }
    };
     ...
     * 증권서열 조회
     */
    MCO01_WS_01001M.prototype.searchPlrnkList = function () {
        var _this = this;
        if (this.rcvNo.trim() == '') {
            this.resetData('');
            this.dialogService.alert('접수번호가 없습니다.');
        ...
        this.http.sendXml(dto, 'APP_CO', 'CoKeyInqPC', 'getListPlcyNo', 'MCO01_WS_01001T', CoInqPCDTO, { ignoreError: true })
            .subscribe(function (result) {
            _this.logger.debug(result);
            if ('MIFI0001' == result.proframeHeader.pfmResponseCode) {
                //this.plrnk = Array.isArray(result.body.coInqCDTO) ? result.body.coInqCDTO[0].plrnk : [result.body.coInqCDTO][0].plrnk;
                _this.plrnk = _this.paramFlag && _this.navParams.data.plrnk ? _this.navParams.data.plrnk : Array.isArray(result.body.coInqCDTO) ? result.body.coInqCDTO[0].plrnk : [result.body.coInqCDTO][0].plrnk;
                _this.searchClcfCdList();
            }
     ...
     * 보상구분조회
     */
    MCO01_WS_01001M.prototype.searchClcfCdList = function () {
        var _this = this;
        if (this.rcvNo.trim() == '') {
            this.resetData('');
            this.dialogService.alert('접수번호가 없습니다.');
        ...
        this.http.sendXml(dto, 'APP_CO', 'CoKeyInqPC', 'getListClcfCd', 'MCO01_WS_01001T', CoInqPCDTO, { ignoreError: true })
            .subscribe(function (result) {
            _this.logger.debug(result);
            if ('MIFI0001' == result.proframeHeader.pfmResponseCode) {
                _this.clcfList = Array.isArray(result.body.coInqCDTO) ? result.body.coInqCDTO : [result.body.coInqCDTO];
                //this.clcfCd = this.clcfList[0].clcfCd; 
                _this.clcfCd = _this.paramFlag && _this.navParams.data.clcfCd ? _this.navParams.data.clcfCd : _this.clcfList[0].clcfCd;
                _this.selectedClcf = _this.clcfCd;
                _this.iptBoolean = true;
                _this.paramFlag = false;
                //this.serachChtmsList();
            }
            else if ('MIFI0005' == result.proframeHeader.pfmResponseCode) {
                //this.serachChtmsList();
     ...
     * 재해서열 변경
     */
    MCO01_WS_01001M.prototype.chngDsrnk = function () {
        if (this.rcvNo != this.rcvNoResult) {
            this.dialogService.alert('접수번호가 변경되었습니다.');
            return;
        }
     ...
     * 보상 변경
     */
    MCO01_WS_01001M.prototype.chngClcf = function () {
        if (this.rcvNo != this.rcvNoResult) {
            this.dialogService.alert('접수번호가 변경되었습니다.');
            return;
        }
     ...
     * 추가서류 POP-UP 오픈
     */
    MCO01_WS_01001M.prototype.more = function () {
        var _this = this;
        if (!this.validationFormCd()) {
            return;
        }
        var params = { formCd: this.formCd, docList: this.docList };
        this.dialogService.showModalDialog(MCO01_CO_01019L, params).then(function (result) {
            _this.logger.debug(result);
            if (typeof result == 'string') {
                _this.formCd = result;
                if (_this.formCd == 'CO991046') {
                    _this.sendTab = 'partner';
                }
                if ('CO991046' != result &&
                    'CO991026' != result &&
                    'CO991027' != result &&
                    ...
                    _this.empType == '01') {
                    //대인
                    _this.formText = _this.docList[_this.formCd].formNm;
                    _this.sendTab = 'custom';
                }
                else if ('CO991045' != result &&
                    'CO991074' != result &&
                    'CO991026' != result &&
                    'CO991159' != result &&
                    _this.empType == '02') {
     ...
     */
    MCO01_WS_01001M.prototype.ptnSrch = function () {
        var _this = this;
        var params = { cmpny: '', name: this.rpairCmpny };
        this.dialogService.showModalDialog(MCO01_CO_01005L, params).then(function (result) {
            _this.logger.debug(result);
            if (result.ptnGnrNm) {
                _this.rpairCmpny = result.ptnGnrNm;
                _this.rpairCmpnyCd = result.ptnCd;
                _this.partnerTelAreaNo = result.faxAreaNo; //팩스지역번호
                _this.partnerTelExchNo = result.faxExchNo; //팩스국번호
                _this.partnerTelSeq = result.faxSeq; //팩스일련번호
                _this.selectedPartnerFax = _this.partnerTelAreaNo;
            }
        });
    };
    /**
     * 발송버튼 클릭
     */
    MCO01_WS_01001M.prototype.send = function () {
        var _this = this;
        if ('' == this.formCd) {
        ...
        if (this.docList[this.formCd] == undefined) {
            ...
            var calngPcNm = this.docList[this.formCd].calngPcNm;
            var calngOpratNm = this.docList[this.formCd].calngOpratNm;
            var inputInfo = this.docList[this.formCd].inputInfo;
            var prntInfo = this.docList[this.formCd].prntInfo;
            var jobKeyInfo = this.docList[this.formCd].jobKeyInfo;
            var jobCfcd = this.docList[this.formCd].jobCfcd;
            var divNm = this.docList[this.formCd].divNm;
            if ('' == calngPcNm.trim()) {
                this.xmlData = '';
                this.send_form_info();
            }
                ...
                this.http.sendXml(dto, 'APP_CO', calngPcNm, calngOpratNm, 'MCO01_WS_01001T', this.docList[this.formCd].prntInfo, { ignoreError: true })
                    .subscribe(function (result) {
                    _this.logger.debug(result);
                    if ('I' == result.proframeHeader.pfmResponseType) {
                        _this.xmlData = result.body;
                        _this.send_form_info();
                    }
                    else {
                        _this.dialogService.alert(result.proframeHeader.pfmResponseCode + ", " + result.proframeHeader.pfmResponseBasc);
                    }
                });
            }
        }
    };
    MCO01_WS_01001M.prototype.send_form_info = function () {
        //자동차진료비지불보증서
        if (this.formCd == 'CO991046') {
            //FAX발송, 심평원전문발송
    ...
    MCO01_WS_01001M.prototype.objectToString = function (obj, name) {
        var str = '';
        var dataKeys = Object.keys(obj);
        var objArr = [];
        var datasetName = obj.constructor.name;
        ...
        if (typeof obj == 'object') {
            ...
            for (var key in dataKeys) {
                ...
                if (typeof obj[dataKeys[key]] == 'object') {
                    var name_1 = dataKeys[key];
                    objArr[name_1] = obj[dataKeys[key]];
    ...
    MCO01_WS_01001M.prototype.docSend = function (param) {
        var _this = this;
        var dto = new ClaimDocMngtInfoCDTO();
        if ('' == this.xmlData) {
        ...
        this.http.sendXml(dto, 'APP_CO', "PrntppMngtPC", "regstClaimDocSendClaimHist", 'MCO01_WS_01001T', ClaimDocMngtInfoCDTO, { ignoreError: true })
            .subscribe(function (result) {
            _this.logger.debug(result);
            if ('I' == result.proframeHeader.pfmResponseType) {
                //this.docSend(result);
                _this.dialogService.alert('발송되었습니다.');
            }
            else {
                _this.dialogService.alert(result.proframeHeader.pfmResponseCode + ", " + result.proframeHeader.pfmResponseBasc);
            }
    ...
    MCO01_WS_01001M.prototype.send_fax_telmsg_form_info_CO991046 = function () {
        var _this = this;
        var hiraPyGurDTO = new HiraPyGurDTO();
        var inGurDto = hiraPyGurDTO.hiraPyGurResDTO;
        var inMngtDto = hiraPyGurDTO.carClncSvcfeePyGurMngtInfoDTO;
        var outdto = hiraPyGurDTO.carClncSvcfeePyGurMngtInfoDTO;
        inGurDto.apvlCfcd = '0';
        ...
        this.http.sendXml(hiraPyGurDTO, 'APP_CO', "CarClncSvcfeePyGurMngtInfoPC", "hiraPyGurTrans", 'MCO01_WS_01001T', HiraPyGurDTO, { ignoreError: true })
            .subscribe(function (result) {
            if ('I' == result.proframeHeader.pfmResponseType) {
                _this.docSend(result);
            }
            else {
                _this.dialogService.alert(result.proframeHeader.pfmResponseCode + ", " + result.proframeHeader.pfmResponseBasc);
            }
    ...
    //자동차진료비지불보증서 발송
    MCO01_WS_01001M.prototype.send_form_info_CO991046 = function () {
        var _this = this;
        var carClncSvcfeePyGurMngtInfoDTO = new CarClncSvcfeePyGurMngtInfoDTO();
        ...
        this.http.sendXml(carClncSvcfeePyGurMngtInfoDTO, 'APP_CO', "CarClncSvcfeePyGurMngtInfoPC", "addCarClncSvcfeePyGurInfo", 'MCO01_WS_01001T', CarClncSvcfeePyGurMngtInfoDTO, { ignoreError: true })
            .subscribe(function (result) {
            _this.logger.debug(result);
            if ('I' == result.proframeHeader.pfmResponseType) {
                _this.docSend(result);
    ...
    MCO01_WS_01001M.prototype.setFormCd = function (param) {
        if (this.validationFormCd()) {
            this.formCd = param;
            this.formText = '추가서류';
            if (param == 'CO991046') {
    ...
    MCO01_WS_01001M.prototype.validationFormCd = function () {
    ...
    // EMAIL : 메일 , 
    // ADDR :주소
    MCO01_WS_01001M.prototype.asMaskedCladMskngAppl = function (type, value) {
        return this.commonService.asMaskedCladMskngAppl(type, value);
    };
    MCO01_WS_01001M.prototype.fillZero = function (str, len) {
        var offset;
        var i;
        var temp_str = str + "";
        ...
        while (offset > 0) {
            switch (offset) {
                ...
                case 5:
                    offset = 0;
                    temp_str = "00000" + temp_str;
                    break;
                case 6:
                    offset = 0;
                    temp_str = "000000" + temp_str;
                    break;
                case 7:
                    offset = 0;
                    temp_str = "0000000" + temp_str;
                    break;
                case 8:
                    offset = 0;
                ...
                case 9:
                    offset = 0;
                    temp_str = "000000000" + temp_str;
                    break;
                case 10:
                    offset = 0;
                    temp_str = "0000000000" + temp_str;
                    break;
                case 11:
                    offset = 0;
                    temp_str = "00000000000" + temp_str;
                    break;
                case 12:
                    offset = 0;
                    temp_str = "000000000000" + temp_str;
                    break;
                case 13:
                    offset = 0;
                    temp_str = "0000000000000" + temp_str;
                    break;
                case 14:
                    offset = 0;
                    temp_str = "00000000000000" + temp_str;
                    break;
                case 15:
                    offset = 0;
                    temp_str = "000000000000000" + temp_str;
                    break;
                default:
                    offset -= 15;
                    temp_str = "000000000000000" + temp_str;
    ...
    return MCO01_WS_01001M;
}());
exports.MCO01_WS_01001M = MCO01_WS_01001M;
